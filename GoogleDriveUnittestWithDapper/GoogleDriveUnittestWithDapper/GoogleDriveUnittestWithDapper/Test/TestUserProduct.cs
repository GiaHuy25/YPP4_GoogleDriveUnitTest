...
namespace GoogleDriveUnittestWithDapper.Test
{
    ...
    public class TestUserProduct
    {
        ...
        public async Task GetUserProductsByUserIdAsync_ValidUserId_ReturnsMultipleUserProductItemDtos()
        {
            ...
            // Act
            var result = await _userProductService!.GetUserProductsByUserIdAsync(userId); // Corrected to service method name
            ...
            Assert.IsTrue(products.Any(p => p.ProductName == "Plan1" && p.Cost == 9.99 && p.Duration == 30)); // Adjusted for double comparison
